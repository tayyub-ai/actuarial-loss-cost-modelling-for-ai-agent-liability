...
normal `f(y|θ)/P(Y>t|θ)`, which is **not conjugate** to the normal prior — so the
strict linear (exact) credibility property fails: the Bayes estimator is no longer
linear in Ȳ.
...
functional E[X]=exp(θ+σ²/2) — that extension is the single highest-leverage piece of
